...
type HandShapeEncryptionMe = 'X' | 'Y' | 'Z';
type HandShapeEncryptionPlayer2 = 'A' | 'B' | 'C';
type HandShapeEncryption = HandShapeEncryptionMe | HandShapeEncryptionPlayer2;
type POINTS = 0 | 1 | 2 | 3 | 6;
type Round = [PlayerHand, PlayerHand];

const handShapeDecryption = new Map<HandShapeEncryption, HandShape>([
  ['A', 'ROCK'],
  ['B', 'PAPER'],
  ['C', 'SCISSOR'],
  ['X', 'ROCK'],
...
const winninConditions = new Map<HandShape, HandShape>([
  ['ROCK', 'SCISSOR'],
  ['PAPER', 'ROCK'],
  ['SCISSOR', 'PAPER'],
])
...
const parseStrategyGuid = (strategyGuid: string): Round[] =>
  strategyGuid
    .trim()
    .split('\n')
    .map(item => item.split(' ') as [HandShapeEncryptionPlayer2, HandShapeEncryptionMe])
    .map(([player2, playerMe]) =>
      [new PlayerHand(player2), new PlayerHand(playerMe)]);


class PlayerHand {
  constructor(shapeEncryption: HandShapeEncryption) {
    this.evaluateShape(shapeEncryption);
  }

  private _shape: HandShape = 'ROCK';
  ...
  compare(playerHand: PlayerHand): void {
    ...
    if (this._shape === playerHand.shape) {
      this._score += score.get('DRAW')!;
      return;
    }
    this._score += score.get('LOSS')!;
  }

  private evaluateShape(handShapeEncryption: HandShapeEncryption) {
    const shape = handShapeDecryption.get(handShapeEncryption)!;
