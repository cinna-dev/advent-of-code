...
class Supplies {
  ...
  addCrate(crate: Crate, stack: number): void {
    if (this._stacks[stack]) {
      this._stacks[stack]?.push(crate)
    } else {
      this._stacks[stack] = [crate]
    }
  }
  ...
  moveCrate(instruction: Instruction): void {
    ...
    const stackFrom = this._stacks[from];
    const crates: Crate[] = stackFrom?.splice(stackFrom.length - count, stackFrom.length);
    const stackTo = this._stacks[to];
    if (!stackTo) {
      this._stacks[to] = [];
    }
    this._stacks[to].push(...crates);
  }
...
class Parser {
  static parse(text: string) {
    ...
    suppliesRows = suppliesRows.reduce<string[]>((acc, row) => {
      const items = [...row.match(/(\[\w]|    )/g) ?? []];
      const fillInColumns = columnCount - (items?.length ?? 0);
      if (fillInColumns > 0) {
