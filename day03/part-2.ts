...
//   (that is, a and refer to different types of items).
//
...
const createStringGroups = (list: string): string[][] =>
  list
    .trim()
    .split('\n')
    .reduce<string[][]>((acc, item, index) =>
        index % 3 === 0
          ? [...acc, [item]]
          : (acc.at(-1)?.push(item), acc)
      , []);

const findCharsIncludedInAllStrings = (strings: string[]): string[] =>
  strings.reduce<string[]>((acc, string) => {
    const restStrings = strings.splice(strings.indexOf(string));
    const chars: string[] = string.split('').filter((char) =>
      restStrings.every(restString => restString.includes(char))
    );
    return Array.from(new Set([...acc, ...chars]));
  }, []);

const sumGroupedBadgesPoints = (groupedBadges: string[][]) =>
  groupedBadges.reduce((acc, groupBadge) =>
      acc + getPriority(groupBadge[0])
    , 0);

executeWithPrompt(__dirname, (items) => {
  const stringGroups = createStringGroups(items);
  const groupBadges: string[][] = stringGroups.map((group) =>
    findCharsIncludedInAllStrings(group)
  );
  const sum = sumGroupedBadgesPoints(groupBadges);
  console.dir(groupBadges);
  console.info(sum);
});
