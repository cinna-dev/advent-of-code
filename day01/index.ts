...
// expedition traditionally goes on foot. As your boats approach land,
...
class Elf {
  ...
  addSnacks(calories: number) {
    this.calories += calories;
  }

  compareCaloriesWith(elf: Elf): number {
    if (this.calories > elf.calories) return 1;
    if (this.calories < elf.calories) return -1;
    return 0;
  }
}

const createElves = (listOfCalories: string) =>
  listOfCalories
    .trim()
    .split('\n')
    .reduce<Elf[]>((acc, calories) =>
      !calories
        ? [...acc, new Elf()]
        : (acc.at(-1)?.addSnacks(+calories), acc)
      , [new Elf()]);

const findElfWithMostCalories = (elves: Elf[]): Elf | undefined =>
  elves.sort((elf1, elf2) =>
    elf1.compareCaloriesWith(elf2)
  ).at(-1);

executeWithPrompt(__dirname, (input: string) => {
  const elves: Elf[] = createElves(input);
  const result = findElfWithMostCalories(elves)?.calories;
  console.log(result);
});
