...
const findContainedPairs = (sectionPairs: Sections[][]): boolean[] =>
  sectionPairs.map(sectionPair =>
    sectionPair[0].contains(sectionPair[1])
    || sectionPair[1].contains(sectionPair[0])
  )
...
executeWithPrompt(__dirname, (text): void => {
  ...
  // console.log(sectionsPairs)
  const containedPairs = findContainedPairs(sectionsPairs);
