...
class Sections {
  ...
  contains(sections: Sections): boolean {
    return sections.min >= this.min && sections.max <= this.max;
  }

  overlap(sections: Sections) {
    return ((this.min >= sections.min) && (this.min <= sections.max))
      || ((this.max >= sections.min) && (this.max <= sections.max));
  }

  private parse(): void {
    const [min, max] = this._ref.split('-');
    this._min = +min;
    this._max = +max;
  }
}

const parseSectionsPairs = (text: string): Sections[][] =>
  text
    .trim()
    .split('\n')
    .map((line) =>
      line
        .split(',')
        .reduce<Sections[]>((acc, ref) =>
            [...acc, new Sections(ref)]
          , [])
    );

const findOverlappingSectionPairs
  = (sectionPairs: Sections[][]): boolean[] =>
  sectionPairs.map(sectionPair =>
    sectionPair[0].overlap(sectionPair[1])
    || sectionPair[1].overlap(sectionPair[0])
  )

const getNumberOfSelfContainingSectionPairs = (containedPairs: boolean[]) =>
  containedPairs.reduce((acc, item) => acc += +item, 0)

executeWithPrompt(__dirname, (text): void => {
  const sectionsPairs: Sections[][] = parseSectionsPairs(text);
  const containedPairs = findOverlappingSectionPairs(sectionsPairs);
  console.log(containedPairs)
  const sum = getNumberOfSelfContainingSectionPairs(containedPairs);
  console.log(sum)
});
